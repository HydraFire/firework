...
import { createPseudoRandom } from "../Math.js"
...
export function createParticleEmitter(options, matrixWorld, time = 0) {
  ...
  const effectRepeatTime = Math.max(
    particleRepeatTime,
    Array.isArray(lifeTime) ? Math.max(...lifeTime) : lifeTime
  )
  ...
  if (config.count > 0 && startIndex + config.count > meshParticleCount) {
    error(
      `run out of particles, increase the particleCount for this ThreeParticleMesh`
    )
  }

  const numParticles =
    count >= 0 ? count : meshParticleCount - mesh.userData.nextIndex
...
function spawn(
  ...
  const frameStyleIndex =
    FRAME_STYLES.indexOf(config.frameStyle) >= 0
  ...
  setVelocityScaleAt(
    ...
    config.velocityScaleMin,
    config.velocityScaleMax
  )
}

...
export function createParticleMesh(options) {
  ...
  const geometry = isMesh
    ? new THREE.InstancedBufferGeometry().copy(config.mesh.geometry)
...
export function updateGeometry(geometry, config) {
  ...
  const opacities = new Float32Array(particleCount * (NUM_KEYFRAMES + 1)).fill(
    1
  ) // opacities over time + frameInfo part 2
  const timings = new Float32Array(particleCount * 4)

  const isInstancedBufferGeometry =
    geometry instanceof THREE.InstancedBufferGeometry
  const bufferFn = isInstancedBufferGeometry
  ...
  geometry.setAttribute(
    "row1",
    new bufferFn(row1s, row1s.length / particleCount)
  )
  geometry.setAttribute(
    "row2",
    new bufferFn(row2s, row2s.length / particleCount)
  )
  geometry.setAttribute(
    "row3",
    new bufferFn(row3s, row3s.length / particleCount)
  )
  geometry.setAttribute(
    "offset",
    new bufferFn(offsets, offsets.length / particleCount)
  )
  geometry.setAttribute(
    "scales",
    new bufferFn(scales, scales.length / particleCount)
  )
  geometry.setAttribute(
    "orientations",
    new bufferFn(orientations, orientations.length / particleCount)
  )
  geometry.setAttribute(
    "colors",
    new bufferFn(colors, colors.length / particleCount)
  )
  geometry.setAttribute(
    "opacities",
    new bufferFn(opacities, opacities.length / particleCount)
  )
  geometry.setAttribute(
    "timings",
    new bufferFn(timings, timings.length / particleCount)
  )
  ...
  if (config.useLinearMotion || config.useRadialMotion) {
    ...
    geometry.setAttribute(
      "velocity",
      new bufferFn(velocities, velocities.length / particleCount)
    )
    geometry.setAttribute(
      "acceleration",
      new bufferFn(accelerations, accelerations.length / particleCount)
    )
  }
  ...
  if (config.useAngularMotion || config.useOrbitalMotion) {
    ...
    geometry.setAttribute(
      "angularvelocity",
      new bufferFn(angularVelocities, angularVelocities.length / particleCount)
    )
    geometry.setAttribute(
      "angularacceleration",
      new bufferFn(
        angularAccelerations,
        angularAccelerations.length / particleCount
      )
    )
  }
  ...
  if (config.useWorldMotion || config.useBrownianMotion) {
    ...
    geometry.setAttribute(
      ...
      new bufferFn(
        worldAccelerations,
        worldAccelerations.length / particleCount
      )
    )
  }
  ...
  if (config.useVelocityScale) {
    ...
    geometry.setAttribute(
      "velocityscale",
      new bufferFn(velocityScales, velocityScales.length / particleCount)
    )
  }
...
export function loadTexturePackerJSON(mesh, config, startIndex, endIndex): any {
  const jsonFilename = mesh.userData.meshConfig.texture.replace(
    /\.[^\.]+$/,
    ".json"
  )
    ...
    .then(atlasJSON => {
      ...
      if (typeof config.atlas === "string") {
        const atlasIndex = Array.isArray(atlasJSON.frames)
          ...
          : Object.keys(atlasJSON.frames).findIndex(
            filename => filename === config.atlas
          )
...
export function setTextureAtlas(material, atlasJSON) {
  ...
  const parts = Array.isArray(atlasJSON.frames)
    ? atlasJSON.frames
    : Object.values(atlasJSON.frames)
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    packedTextureAtlas[j] = packUVs(
      frame.x / imageSize.w,
      frame.y / imageSize.h
    )
    packedTextureAtlas[j + 1] = packUVs(
      frame.w / imageSize.w,
      frame.h / imageSize.h
    )
  }
...
export function setAtlasIndexAt(geometry, i, atlasIndex) {
  ...
  opacities.setW(
    i,
    Math.floor(packB) +
    Math.max(0, atlasIndex) / 64 +
    ((packB * 262144) % 4096) / 262144
  )
}
...
export function setVelocityScaleAt(
  geometry,
  i,
  velocityScale,
  velocityMin,

import { Component, SystemStateComponent, Types, System } from 'ecsy';
import { Object3DComponent } from 'ecsy-three';
import { Transform, Parent } from 'ecsy-three/extras';
import { DataTexture, RGBFormat, TextureLoader, InstancedBufferGeometry, BufferGeometry, ShaderLib, UniformsUtils, ShaderMaterial, Mesh, Points, Float32BufferAttribute, Matrix4, Texture, NormalBlending, InstancedBufferAttribute, MathUtils, Vector3, Quaternion, Euler } from 'three';
...
function createParticleMesh(options) {
    ...
    const geometry = isMesh
        ? new InstancedBufferGeometry().copy(config.mesh.geometry)
        : new BufferGeometry();
...
function setAtlasIndexAt(geometry, i, atlasIndex) {
    ...
    opacities.setW(i, Math.floor(packB) +
...
function createParticleEmitter(options, matrixWorld, time = 0) {
    ...
    textureFrame = config.textureFrame
        ? config.textureFrame
        : mesh.userData.meshConfig.textureFrame;
...
function spawn(geometry, matrixWorld, config, index, spawnTime, lifeTime, repeatTime, textureFrame, seed, rndFn) {
    ...
    const particleLifeTime = Array.isArray(lifeTime)
        ? rndFn() * (lifeTime[1] - lifeTime[0]) + lifeTime[0]
        : lifeTime;
    ...
    const endFrame = frames.length > 1
        ...
        : frames.length > 0
            ? frames[0]
            : textureFrame.cols * textureFrame.rows - 1;
    const frameStyleIndex = FRAME_STYLES.indexOf(config.frameStyle) >= 0
        ? FRAME_STYLES.indexOf(config.frameStyle)
        : 0;
